...
-- test_partitioned_premake
-- premake is passed through to pg_partman and defaults to pg_partman's own 4
SELECT pgmq.create_partitioned('test_premake_default', '10', '100');
SELECT premake FROM part_config WHERE parent_table = 'pgmq.q_test_premake_default';
SELECT pgmq.create_partitioned('test_premake_custom', '10', '100', 2);
SELECT premake FROM part_config WHERE parent_table = 'pgmq.q_test_premake_custom';

-- msg_id on a partitioned queue is GENERATED BY DEFAULT, so pg_partman can move
-- rows out of the default partition with their ids intact
SELECT identity_generation FROM information_schema.columns
 WHERE table_schema = 'pgmq' AND table_name = 'q_test_premake_custom' AND column_name = 'msg_id';

-- metrics report the default partition: empty while every message has a partition
...
SELECT default_partition_length IS NULL FROM pgmq.metrics('test_premake_plain');

SELECT pgmq.drop_queue('test_premake_default');
SELECT pgmq.drop_queue('test_premake_custom');
SELECT pgmq.drop_queue('test_premake_plain');
